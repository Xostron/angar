...
function web(code, obj, oData) {
	...
	const { type, sel, vlvId, setpoint, name } = obj
	if (type !== 'valve') return { noLog: true }
	let bld, sec, vlv

	for (const key in obj) {
		if (['type', 'sel', 'vlvId', 'setpoint', 'name'].includes(key)) continue
	...
	let title = `${bld.name} ${bld.code}. ${sec.name}. ${
		vlv.type == 'in' ? 'Приточный клапан: ' : 'Выпускной клапан: '
	}`
	...
	return { bId: bld._id, sId: sec._id, title, value: sel, name }
}
