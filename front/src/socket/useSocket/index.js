...
export default function useSocket() {
	const { updateOn } = useSocketStore(({ updateOn }) => ({ updateOn }));
	...
	useEffect(() => {
		...
		// socket.io.on('ping', () => defEvent('ping'));
		socket.io.on('reconnect_failed', () => defEvent('reconnect_failed'));
		socket.io.on('reconnect_attempt', (attempt) =>
			defEvent('reconnect_attempt', attempt)
		);
		socket.io.on('reconnect_error', (error) =>
			defEvent('reconnect_error', error)
		);
		return () => {
			socket.off('connect', connect);
			socket.off('disconnect', disconnect);
			socket.off('connect_error', (error) =>
				defEvent('connect_error', error)
			);
