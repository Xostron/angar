...
const useOutputStore = create((set, get) => ({
  ...
  setO(out1, out2, type, vlvId) {
    ...
    output[out1.idB] = {
      ...output[out1.idB],
      [out1.idM]: {
        ...output?.[out1.idB]?.[out1.idM],
        [out1.channel]: out1.value,
      },
    };
    if (out2)
      output[out2.idB] = {
        ...output[out2.idB],
        [out2.idM]: {
          ...output?.[out2.idB]?.[out2.idM],
          [out2.channel]: out2.value,
        },
      };
    set({ output });
  ...
  setSens(obj) {
    ...
    sens[obj.build] = {
      ...sens[obj.build],
      [obj._id]: { ...sens?.[obj.build]?.[obj._id], [key]: obj[key] },
    };
    set({ sens });
  },
