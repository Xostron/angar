...
function extra(building, section, obj, s, se, m, alarm, resultFan, data, type = 'section', state = 'on') {
	for (const key in def[type][state]) {
		// Аккумулятор для хранения промежуточных вычислений доп функций
		let code = key
		if (key === 'coOn' || key === 'coAuto') code = 'co2'
		else code = key
...
function extraClear(building, section, obj, s, se, m, alarm, resultFan, data, type = 'section', state = 'on') {
	for (const key in def[type][state]) {
	def[type][state][key](building, section, null, null, null, null, null, null, null, null, null, true)
	}
}
