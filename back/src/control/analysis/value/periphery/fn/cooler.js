	...
	cooler.forEach((clr) => {
		...
		clr.solenoid.forEach((el) => {
			...
			if(result?.[sigId] === undefined) {
				console.log('Нет сигнала от соленоида используем альтернативу')
		...
		fan.filter((el) => el.owner.id === clr._id).forEach((f) => {
			result[clr._id].fan[f._id] ??= {};
			result[clr._id].fan[f._id].state = stateF(f, equip, result, retain);
		});
...
function state(o) {
	...
	const f =
		Object.values(fan ?? {}).some((el) => el?.state === 'run') ?? false;
