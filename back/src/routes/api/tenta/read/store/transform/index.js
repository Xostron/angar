...
async function transform(bldId, secId) {
	try {
		...
		const [sensor, fan, heating, valve, section, building, cooler, device] =
			await Promise.all(p)
		// Тип склада
		...
		console.log(888, result[bldId + 'push'])
		...
		result[bldId + 'tout'] = {
			value: data?.total?.[bldId]?.tout?.min?.toFixed(1) ?? undefined,
			state: data?.total?.[bldId]?.tout?.state,
		}

		// Влажность улицы склада
		result[bldId + 'hout'] = {
			value: data?.total?.[bldId]?.hout?.max?.toFixed(1) ?? undefined,
			state: data?.total?.[bldId]?.hout?.state,
		}

		// Абсолютная влажность улицы
		result[bldId + 'ah'] = {
			value: data?.humAbs?.out?.[bldId],
			state: checkS(data?.total?.[bldId]?.tout?.state, data?.total?.[bldId]?.hout?.state),
		}
