...
function fnTransform(bld, data, section, fan, result) {
	...
	result[bld._id + 'mode'] =
		store.value?.building?.[bld._id]?.submode?.[0] ?? data?.retain?.[bld._id]?.automode ?? null
	result[bld._id + 'count'] =
		data?.retain?.[bld._id]?.drying?.count ?? data?.retain?.[bld._id]?.drying?.acc ?? 0
	...
	// Таймеры запретов
	const timer = Object.values(data?.alarm?.timer?.[bld._id] ?? {}).map((el) => ({
		code: el?.type,
		msg: el?.msg,
	}))
