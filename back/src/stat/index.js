...
/**
 * Статистика - сбор данных каждый цикл по изменению
...
async function statOnTime() {
	while (true) {
		...
		// Значения датчиков после анализа (мин или макс)
		pLogTotal(store?.value?.total, data.building)
		// Значения каждого физического датчика
		pLogSensor(data, data.sensor, store.value, 'sensor')
		// Значения сигналов binding c type = 'ai'
		pLogBindingAI(data, data.binding, store.value, 'sensor')
		// Значения моточасов склада
		pLogSensor(data, data.building, null, 'hour')
